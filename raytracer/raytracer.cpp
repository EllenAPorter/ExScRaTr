...
int step_compose_image::execute
	...
	Multispectral3D *debug = new Multispectral3D[rt.image_width*rt.image_height];
	...
	for(int fragment = 0; fragment < rt.number_fragments; fragment++) {
		...
		for(int i = 0; i < pixels[fragment].size(); i++) {
			...
			debug[pixels[fragment][i]->i+rt.image_width*pixels[fragment][i]->j] =
					Multispectral3D(((double)fragment)/8.0);
		}
	}
	...
	// Produce an image showing which pixel was computed by which fragment
	std::stringstream frame_id_debug;
	frame_id_debug << "./frames/frame_" << current_frame << "_debug.ppm";
	std::ofstream ofs_debug(frame_id_debug.str().c_str(), std::ios::out |
			std::ios::binary);
	ofs_debug << "P6\n" << rt.image_width << " " << rt.image_height << "\n255\n";
	for(int i = 0; i < rt.image_width*rt.image_height; i++) {
		ofs_debug <<
			(unsigned char)(std::min(1.0, debug[i].r) * 255) <<
			(unsigned char)(std::min(1.0, debug[i].g) * 255) <<
			(unsigned char)(std::min(1.0, debug[i].b) * 255);
 ...
 //   CnC::debug::trace(cnc_ray_tracer.execution_time);
    ...
    pushStaticScene(cnc_ray_tracer.static_scene);
  	...
  	cnc_ray_tracer.luminaires.push_back(new Luminaire(
  			Multispectral3D(0.1, 0.1, 0.1), Sphere(Point3D(4, 20, 10), 3), 4));
