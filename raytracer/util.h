...
public:
	...
	Multispectral3D(double rgb):r(rgb), g(rgb), b(rgb){};
...
public:
	Point3D o; // origin
	Point3D p; // clockwise from origin
	Point3D q;
	Vector3D n;
...
void pushBunny(std::vector<Primitive*> &list, double rotation) {
	...
	if (myfile.is_open()) {
		...
		while ( getline (myfile,line) ) {
			...
			if(s == "v") {
				...
				double x = stringToDouble(s) * 100;

				getline(iss, s, ' ');
				double y = (stringToDouble(s)) * 100;

				getline(iss, s, ' ');
				double z = (stringToDouble(s)) * 100;


				Point3D p = transform * Point3D(x, y, z);
				...
				getline(iss, s, ' ');
				double y = (stringToDouble(s));

				getline(iss, s, ' ');
				double z = (stringToDouble(s));
				...
				std::string v0 = s.substr(0, s.find('/'));
				std::string n0 = v0.substr(0, v0.length());

				getline(iss, s, ' ');
				std::string v1 = s.substr(0, s.find('/'));
				std::string n1 = v1.substr(0, v1.length());

				getline(iss, s, ' ');
				std::string v2 = s.substr(0, s.find('/'));
				std::string n2 = v2.substr(0, v2.length());

				int o = stringToInteger(v0) - 1;
				int p = stringToInteger(v1) - 1;
				int q = stringToInteger(v2) - 1;

				int oN = stringToInteger(n0) - 1;
				int pN = stringToInteger(n1) - 1;
				int qN = stringToInteger(n2) - 1;

				// Calcualte a face normal
				Vector3D normal = (((*normals[oN]) + (*normals[pN]) +
						(*normals[qN])) / 3).normalize();

				list.push_back(new Primitive(new Triangle(*points[o],
						*points[p], *points[q]), gold));
...
void pushStaticScene(std::vector<Primitive*> &static_scene) {
	...
	Material *blue_glass = new Material(
	  		Multispectral3D(0.36, 0.32, 1.00),	// ambient
	  		Multispectral3D(0.36, 0.32, 1.00),	// diffuse
	  		Multispectral3D(0.2, 0.2, 0.2),			// specular
	  		7.8974,															// shininess
	  		1.54,																// refraciton index
	  		0.2,																// reflective %
	...
	static_scene.push_back(new Primitive(
			new Sphere(Point3D(0, -10004, -20), 10000), glass3));
	static_scene.push_back(new Primitive(
			new Sphere(Point3D(3.5, -1, -10),  2.5), blue_glass)); // blue glass
	static_scene.push_back(new Primitive(
			new Sphere(Point3D(-24, 0, -62), 16), glass4)); // gray
//	static_scene.push_back(new Primitive(
